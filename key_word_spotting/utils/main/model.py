...
def my_fft_to_obtain_tf_time(x1,x2,s):
    # s in seconds
    sample_rate = 16000
    window = 'hann'
    n_fft = 480
    ...
    TF = np.divide(FFT2_mean,FFT1_mean)
    return [FFT2_mean, FFT1_mean, TF[:,None]/TF.max()]


def my_obtain_tf_usb_with_time(x1,x2,s):
    FFT12_mean = my_fft_to_obtain_tf_time(x1,x2,s)
    FFT12_mean[2][:5] = 0
    return [FFT12_mean[0], FFT12_mean[1], FFT12_mean[2]]
     

def my_get_tf_phyaug(t):
    flist = [
        "/home/phuc/Desktop/main/PhyAug/key_word_spotting/data/recorded_dataset/white_noise/white-noise-true-20min.wav",
        "/home/phuc/Desktop/main/PhyAug/key_word_spotting/data/recorded_dataset/white_noise/ATR-white.wav",
        "/home/phuc/Desktop/main/PhyAug/key_word_spotting/data/recorded_dataset/white_noise/clipon-white.wav",
    ...
    tf_atr = my_fft_to_obtain_tf_time(flist[0],flist[1],t) #+ stft_ave_no_DC(flist[6])
    tf_clipon = my_fft_to_obtain_tf_time(flist[0],flist[2],t) #+ stft_ave_no_DC(flist[7])
    tf_maono = my_fft_to_obtain_tf_time(flist[0],flist[3],t) #+ stft_ave_no_DC(flist[8])
    tf_USB = my_obtain_tf_usb_with_time(flist[0],flist[4],t) #+ stft_ave_no_DC(flist[9])
    tf_USBplug = my_obtain_tf_usb_with_time(flist[0],flist[5],t) #+ stft_ave_no_DC(flist[10])
    return [tf_atr,tf_clipon,tf_maono,tf_USB,tf_USBplug]
...
class SpeechDataset(data.Dataset):
    ...
    def collate_fn_transform_with_tf(self, data):
        x = None
        y = []
        for audio_data, label in data:
            if self.audio_preprocess_type == "MFCCs":
                print(audio_data.shape)
                for data in self.audio_processor.compute_mfccs_transform_with_tf(audio_data):
                    audio_tensor = torch.from_numpy(data.reshape(1, -1, 40))
    ...
    def __getitem__(self, index):
        if index >= len(self.audio_labels):
            return self.audio_processor.compute_spectrogram(self.load_audio(None, silence=True)), 0
        return self.inputs[index], self.labels[index] 
    
    def audio_files_to_wav(self):
        for index in range(len(self.audio_files)):
            self.inputs.append(self.audio_processor.compute_spectrogram(self.load_audio(self.audio_files[index])))
            self.labels.append(self.audio_labels[index])
...
class TensorDataset(data.Dataset):
    ...
    #         self.inputs.append(torch.from_numpy(ds.audio_processor.compute_mel(ds.inputs[i]).reshape(1, -1, 40)))
    #         self.labels.append(torch.tensor(ds.labels[i]))
