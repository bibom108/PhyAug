...
class AudioPreprocessor(object):
    def __init__(self, sr=16000, n_dct_filters=40, n_mels=40, f_max=4000, f_min=20, n_fft=480, hop_ms=10, alpha=[], tf=[], sigma_level = 0):
        super().__init__()
        self.n_mels = n_mels
    ...
    def compute_mfccs_transform_with_tf(self, data):
        ...
        index_tf = random.randint(0,4)
        for index_alpha in range(len(self.alpha)):
            noise = np.random.randn(*self.alpha[index_alpha].shape) * self.sigma_level
            cur_data = np.exp(self.alpha[index_alpha] + noise) * self.tf[index_tf][2] * data
            cur_data = cur_data**2
            # compute mfcc
            cur_data = librosa.feature.melspectrogram(S=cur_data,
                                                sr=self.sr,
                                                fmin=self.f_min,
                                                fmax=self.f_max,
                                                n_mels=self.n_mels)
            cur_data[cur_data > 0] = np.log(cur_data[cur_data > 0])
            cur_data = [np.matmul(self.dct_filters, x) for x in np.split(cur_data, cur_data.shape[1], axis=1)]
            cur_data = np.array(cur_data, order="F").astype(np.float32)
            ## adding z-score normalize
            mean = np.mean(cur_data)
            std = np.std(cur_data)
            if std != 0:
                cur_data = (cur_data - mean)/std
            res.append(cur_data)
        return res
    ...
    def compute_mfccs_with_tf(self, data):
        ...
        # print("index = ",index)
        # print("tf is ",self.tf)
        # noise = np.random.randn(*self.tf[index].shape) * self.sigma_level
