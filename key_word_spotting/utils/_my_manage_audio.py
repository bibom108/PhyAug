...
class AudioPreprocessor(object):
    def __init__(self, sr=16000, n_dct_filters=40, n_mels=40, f_max=4000, f_min=20, n_fft=480, hop_ms=10, tf=[], sigma_level = 0):
        super().__init__()
        self.n_mels = n_mels
    ...
    def compute_mfccs(self, data):
        data = np.abs(librosa.stft(data,
                            ...
                            n_fft=self.n_fft))
        sigma = np.random.randn(*data.shape) * self.sigma_level
        data = data + sigma # multiply each column of spect with TF
        data = data**2
    ...
    def compute_mfccs_with_tf(self, data):
        ...
        # print("index = ",index)
        # print("tf is ",self.tf)
        sigma = np.random.randn(*self.tf[index].shape) * self.sigma_level
        data = data*(self.tf[index] + sigma) # multiply each column of spect with TF
        data = data**2
        # compute mfcc
        data = librosa.feature.melspectrogram(S=data,
                                              sr=self.sr,
